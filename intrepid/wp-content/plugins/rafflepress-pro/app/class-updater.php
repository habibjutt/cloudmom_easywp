...
class RafflePress_Updater {
	...
	public function __construct( array $config ) {
		...
		if ( ! current_user_can( apply_filters( 'rafflepress_update_plugins_capability', 'update_plugins' ) ) ) {
			return;
		}
	...
	public function update_plugins_filter( $value ) {
		...
		if ( isset( $this->update->new_version ) && version_compare( $this->version, $this->update->new_version, '<' ) ) {
			...
			$item = (object) array(
				'id'            => 'rafflepress-pro/rafflepress-pro.php',
				'slug'          => 'rafflepress-pro',
				'plugin'        => 'rafflepress-pro/rafflepress-pro.php',
				'new_version'   => RAFFLEPRESS_PRO_VERSION,
				'url'           => '',
				'package'       => '',
				'icons'         => array(),
				'banners'       => array(),
			...
			$value->no_update['rafflepress-pro/rafflepress-pro.php'] = $item;
		}
	...
	public function perform_remote_request( $action, $body = array(), $headers = array(), $return_format = 'json' ) {
		...
		if ( ! empty( $response_body->valid ) && $response_body->valid == 'true' ) {
			// Store API key
			update_option( 'rafflepress_api_message', $response_body->message );
			update_option( 'rafflepress_a', true );
			update_option( 'rafflepress_license_name', $response_body->plugin_name );
			update_option( 'rafflepress_per', $response_body->per );
			$response = array(
				'status' => 'true',
			...
			if ( $response_body->message != 'Unauthenticated.' ) {
