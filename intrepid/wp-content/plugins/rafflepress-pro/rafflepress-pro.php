...
define( 'RAFFLEPRESS_PRO_VERSION', '1.12.14' );
...
function rafflepress_pro_activation() {
	...
	if ( true === rafflepress_pro_set_up_upload_dir( $path, $cache_file ) ) {
		$initial_location = RAFFLEPRESS_PRO_PLUGIN_PATH . 'resources/data-templates/articles.json';
		copy( $initial_location, $cache_file );
	}

	// Set cron to fetch help docs.
	if ( ! wp_next_scheduled( 'rafflepress_pro_fetch_help_docs' ) ) {
		if ( RAFFLEPRESS_PRO_BUILD === 'pro' ) {
			wp_schedule_event( time() + 7200, 'weekly', 'rafflepress_pro_fetch_help_docs' );
		} else {
			wp_schedule_event( time(), 'weekly', 'rafflepress_pro_fetch_help_docs' );
		}
	}
}
